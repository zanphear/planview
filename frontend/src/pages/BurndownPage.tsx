...
import { useBurndown } from '../api/queries/stats';
...
export function BurndownPage() {
  ...
  const burndownQuery = useBurndown(workspace?.id, days, projectId || undefined);

  // ── Four states ─────────────────────────────────────────────────────────
  if (burndownQuery.isPending) return <LoadingSpinner />;

  if (burndownQuery.isError) {
    return (
      <div className="p-4 sm:p-6 max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-sm mb-4" style={{ color: 'var(--color-text-secondary)' }}>
            Couldn't load the burndown data.
          </p>
          <button
            onClick={() => burndownQuery.refetch()}
  ...
  const isEmpty = points.every((p) => p.created === 0 && p.completed === 0 && p.remaining === 0);
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      ...
      {isEmpty ? (
        ...
        >
          ...
          <p className="text-sm mb-4" style={{ color: 'var(--color-text-secondary)' }}>
            Create and complete tasks to see your burndown take shape.
          </p>
          <a
            href="/projects"
                  ...
                  {[...points].reverse().map((p) => (
                    ...
                    >
                      ...
                      <td
                        ...
                        style={{
                          color:
                            p.created > 0 ? 'var(--color-primary)' : 'var(--color-text-secondary)',
                        }}
                      >

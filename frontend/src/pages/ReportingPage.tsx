...
import { usePeopleStats } from '../api/queries/stats';
...
export function ReportingPage() {
  ...
  const peopleQuery = usePeopleStats(workspace?.id);
  ...
  const summaryCards = data
    ? [
        ...
        },
        {
          ...
          value: data.compliance.expiring_soon + data.compliance.expired,
          icon: Shield,
          colour: 'bg-red-500',
        },
        {
          label: 'Active Candidates',
          value: data.recruitment.active,
          icon: UserPlus,
          colour: 'bg-teal-500',
        },
        {
          label: 'Meeting Completion',
          value: `${data.meetings.completion_rate}%`,
          icon: MessageSquare,
          colour: 'bg-indigo-500',
        },
        {
          label: 'Skills Tracked',
          value: data.competencies.total_skills,
          icon: Award,
          colour: 'bg-cyan-500',
        },
        {
          label: 'Kudos (30 days)',
          value: data.wellbeing.recent_kudos,
          icon: Heart,
          colour: 'bg-pink-500',
        },
      ]
      ...
      {peopleQuery.isPending ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="mb-4 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
            Add team members to start tracking metrics across your modules.
          </p>
          <NavLink
            to="/people"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {Object.keys(data.people.by_department).length > 0 && (
                <Card title="People by Department" icon={Users}>
                  <BarChart
                    bars={Object.entries(data.people.by_department).map(([dept, count], i) => ({
                      label: dept,
                      value: count,
                      colour: [
                        COLOURS.blue,
              ...
              {isEnabled('objectives') && (
                <Card title="Objectives" icon={Target}>
                  <div className="flex items-center gap-6">
                    <DonutChart
                      segments={Object.entries(data.objectives.by_status).map(
                        ([status, count]) => ({
                  ...
                  <div className="flex items-center gap-6">
                    <DonutChart
                      segments={[
                        ...
                        {
                          label: 'Expiring Soon',
                          value: data.compliance.expiring_soon,
              ...
              {isEnabled('competencies') && data.competencies.total_skills > 0 && (
                <Card title="Competencies" icon={Award}>
                  <div className="flex items-center gap-6">
                    <BarChart
                      bars={Object.entries(data.competencies.by_level).map(([level, count]) => ({
                        label: `Level ${level}`,
                        value: count,
                        colour:
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    ...
                    <div>
                      <div className="text-2xl font-bold" style={{ color: COLOURS.green }}>
                        {data.leave.approved_this_month}
                      </div>
                      <div
                        className="text-xs mt-1"
                        style={{ color: 'var(--color-text-secondary)' }}
              ...
              {isEnabled('recruitment') && (
                <Card title="Recruitment Pipeline" icon={UserPlus}>
                  <BarChart
                    bars={Object.entries(data.recruitment.by_stage).map(([stage, count]) => ({
                      label: stage,
                      value: count,
              ...
              {isEnabled('development') && (
                <Card title="Development" icon={GraduationCap}>
                  <div className="flex items-center gap-8">
                    <ProgressRing
                      value={data.development.completion_rate}
                      size={80}
                    ...
                    <div>
                      <div className="text-2xl font-bold" style={{ color: COLOURS.amber }}>
                        {data.reviews.avg_rating !== null
                          ? data.reviews.avg_rating.toFixed(1)
                          : ', '}
                      </div>
                      <div
                        className="text-xs mt-1"
                  ...
                  <div className="flex items-center gap-6">
                    <div className="flex gap-3">
                      {data.wellbeing.avg_morale !== null && (
                        <ProgressRing
                          value={(data.wellbeing.avg_morale / 5) * 100}
                          size={64}
                          colour={COLOURS.pink}
                          label="Morale"
                        />
                      ...
                      {data.wellbeing.avg_workload !== null && (
                        <ProgressRing
                          value={(data.wellbeing.avg_workload / 5) * 100}
                          size={64}
                          colour={COLOURS.amber}
                          label="Workload"
                        />
                      ...
                      {data.wellbeing.avg_support !== null && (
                        <ProgressRing
                          value={(data.wellbeing.avg_support / 5) * 100}
                          size={64}
                          colour={COLOURS.green}
                          label="Support"
                        />
              ...
              {isEnabled('onboarding') && (
                <Card title="Onboarding" icon={ClipboardList}>
                  <div className="flex items-center gap-8">
                    <ProgressRing
                      value={data.onboarding.avg_progress}
                      size={80}
                      colour={COLOURS.teal}
                      label="Progress"
                    />

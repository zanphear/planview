...
export function BugReportModal({ onClose }: BugReportModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await feedbackApi.create(workspace.id, { type: 'bug', title: title.trim(), description: description.trim() });
      Toast.show('Bug report submitted');
      onClose();
    } catch {
      Toast.show('Failed to submit bug report');
    }
    setSubmitting(false);
  };
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold" style={{ color: 'var(--color-text)' }}>Report a Bug</h3>
          </div>
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t" style={{ borderColor: 'var(--color-border)' }}>
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm rounded-lg hover:bg-[var(--color-grey-2)] transition-colors"

...
export function FeatureRequestModal({ onClose }: FeatureRequestModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await feedbackApi.create(workspace.id, { type: 'feature', title: title.trim(), description: description.trim() });
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold" style={{ color: 'var(--color-text)' }}>Request a Feature</h3>
          </div>

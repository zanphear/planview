      ...
      {bars.map((bar, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-2">
            <span className="text-xs w-20 truncate shrink-0" style={{ color: 'var(--color-text-secondary)' }}
              ...
              <svg width="100%" height={barHeight} preserveAspectRatio="none">
                {(() => {
                  ...
                  return bar.segments.map((seg, j) => {
                    ...
                    const el = (
                      <rect key={j} x={`${x}%`} y={0} width={`${pct}%`} height={barHeight} rx={j === 0 ? 3 : 0} fill={seg.colour}>
            ...
            <span className="text-xs w-8 text-right shrink-0" style={{ color: 'var(--color-text-secondary)' }}>{total}</span>
          </div>
        );
      })}
    </div>

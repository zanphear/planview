...
export function ProgressRing({ value, size = 48, strokeWidth = 5, colour = 'var(--color-primary)', label }: ProgressRingProps) {
    ...
    <div className="inline-flex flex-col items-center gap-1">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <text x={center} y={center + 1} textAnchor="middle" dominantBaseline="middle"
          fontSize={size / 4} fontWeight="bold" fill="var(--color-text)">
          {Math.round(value)}%
        </text>
      </svg>
      {label && (

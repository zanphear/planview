...
export interface PeopleStats {
  ...
  compliance: { valid: number; expiring_soon: number; expired: number; total: number; by_type: Record<string, number> };
  competencies: { total_skills: number; by_level: Record<string, number>; total_assignments: number };
  ...
  development: { total_plans: number; active_plans: number; total_goals: number; completed_goals: number; completion_rate: number };
  reviews: { total_cycles: number; total_reviews: number; avg_rating: number | null; completed: number };
  wellbeing: { avg_morale: number | null; avg_workload: number | null; avg_support: number | null; total_kudos: number; recent_kudos: number };
